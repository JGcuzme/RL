...
class get_Reward(object):
    ...
    def calculate_reward_single_graph(self, graph_batch, lambda1, lambda2):
        ...
        if self.score_type == 'BIC':
            BIC = np.log(np.sum(RSS_ls) / self.n_samples) + np.sum(graph_batch) * self.bic_penalty
            # BIC = np.log(np.sum(RSS_ls) / self.n_samples) + np.sum(graph_batch) * self.bic_penalty / self.maxlen
        elif self.score_type == 'BIC_different_var':
            BIC = np.sum(np.log(np.array(RSS_ls) / self.n_samples)) + np.sum(graph_batch) * self.bic_penalty
        self.d_BIC = BIC
        score = self.score_transform(BIC)

        cycness = np.trace(matrix_exponential(np.array(graph_batch))) - self.maxlen
        reward = score + lambda1 * float(cycness > 1e-5) + lambda2 * cycness   # 原始
        # reward = score + lambda1 + lambda2 * cycness   # 原始
